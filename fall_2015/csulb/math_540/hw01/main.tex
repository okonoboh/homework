...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%L.2%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
   \item[]        \textbf{Lemma 2.} \textit{Let $r$ and $s$ be the generators of
                  ...
                  \begin{equation} \label{L1_1}
                     r^is = sr^{-i}
                  \end{equation}
                  \textit{holds for all non-negative integers $i$.}

                  \textbf{Proof.} Proceed by induction on $i$. The base
                  case($i = 0$) holds trivially (additionally, \eqref{L1_1}
                  also holds by observing the relations in the the presentation
                  of $D_{2n}$). Now, for the inductive hypothesis, assume that
                  \eqref{L1_1} also holds for some positive integer $k$. Then it
                  follows that
                  \begin{align*}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%L.3%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
   \item[]        \textbf{Lemma 3.} \textit{Let $n$ be a positive integer.
                  If $a_1$, $a_2$, $\ldots$, $a_m$, are mutually commuting
                  elements of a group, then}
                  \begin{equation} \label{L3_1}
                     (a_1a_2\ldots a_m)^n = {a_1}^n{a_2}^n\ldots{a_m}^n
                  \end{equation}
                  \textit{for each positive integer $m$.}
    
      \textbf{Proof.} Suppose $a$ and $b$ be commuting elements of some group.
      We shall first show by induction on $n$ that
      \begin{equation} \label{L3_2}
         (ab)^n = a^nb^n.
      \end{equation}
      The base case ($n=1$) is a trivial since $(ab)^1 = ab = a^1b^1$. For the
      inductive hypothesis, assume that \eqref{L3_2} holds for some positive
      integer $k$. Then it follows that
      \begin{align*}
         (ab)^{k+1} &= (ab)^kab &[\text{Exercise 1.1.19(a)}] \\
            &= (a^kb^k)ab &[\text{Inductive hypothesis}] \\
            &= a^k(b^ka)b &[\text{Associativity}] \\
            &= a^k(ab^k)b &[\text{$a$ commutes with $b$}] \\
            &= (a^ka)(b^kb) &[\text{Associativity}] \\
            &= a^{k+1}b^{k+1} &[\text{Exercise 1.1.19(a)}],
      \end{align*}
      so that \eqref{L3_2} holds for $k+1$. Hence, by induction, \eqref{L3_2}
      holds for each positive integer $n$. Now we want to prove \eqref{L3_1} by
      induction on $m$. The base case $m=1$ is trivial and we have just shown,
      in \eqref{L3_2},
      that the case $m = 2$ also holds. So assume, for the inductive hypothesis,
      that \eqref{L3_1} holds for some positive integer $j$. Now suppose that
      $b_1$, $b_2$, $\ldots$, $b_j$, $b_{j+1}$, are mutually commuting elements
      of some group, and let $p$ be a positive integer. Since the $b_i$ elements
      are mutually commuting, it follows that the order of the elements in the
      product is irrelevant; that is,
      $$(b_1b_2\ldots b_j) b_{j+1} = b_{j+1}(b_1b_2\ldots b_j).$$
      Apply \eqref{L3_2} to the preceding equality (with $n = p$,
      $a = (b_1b_2\ldots b_j)$, and $b = b_{j+1}$) to get
      $$[(b_1b_2\ldots b_j) b_{j+1}]^p = (b_1b_2\ldots b_j)^p {b_{j+1}}^p.$$
      Finally we have that
      \begin{align*}
         (b_1b_2\ldots b_jb_{j+1})^p &= (b_1b_2\ldots b_j)^p {b_{j+1}}^p \\
            &= {b_1}^p{b_2}^p\ldots{b_j}^p{b_{j+1}}^p.
               &[\text{Inductive hypothesis}]
      \end{align*}
      That is, \eqref{L3_1} holds for $j+1$, and we can conclude, by induction,
      that it holds for each integer $m$. \qed
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%1.6%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
   \item[1.1.6]   Determine which of the following sets are groups under
      ...
      \begin{enumerate}
         \item We claim that the set
               ...
               $r = \frac{r_1}{r_2}$ and $s = \frac{s_1}{s_2}$ for some
               integers $r_1$, $s_1$, and odd integers $r_2$ and $s_2$, such
               that $\gcd(r_1, r_2) = \gcd(s_1, s_2) = 1$. It follows that
               \begin{align*}
                  r + s &= \frac{r_1}{r_2} + \frac{s_1}{s_2} \\
                        &= \frac{r_1s_2 + r_2s_1}{r_2s_2}.
               \end{align*}

               Since $r_2$ and $s_2$ are both odd, it must necessarily be the 
               case that $r_2s_2$ is also odd. In order words, $r_2s_2$ contains 
               ...
               We observe that $0/1$ is the identity element in $S$ because
               it is an element of $S$ and $0 + s = s + 0 = s$ for every
               $s \in S$. Next, we notice that
               $\gcd(r_1, r_2) = \gcd(-r_1, r_2) = 1$, so that
               $\frac{-r_1}{r_2} \in S$. Since
               $$\frac{-r_1}{r_2} + \frac{r_1}{r_2} =
                  \frac{r_1}{r_2} + \frac{-r_1}{r_2} = 0,$$
               and since $r$ was arbitrarily chosen it follows that $S$ is
               closed under inverses. Since $S \subset \Q$, and since $\Q$ is
               associative under addition, it follows that $S$ is also
               associative under addition. Thus $S$ satisfies the group axioms,
               so that $(S, +)$ is a group. \qed
         \item The set
   ...
   \item[1.1.19]  Let $x \in G$ and let $a, b \in \Z^+$.
      ...
      \begin{enumerate}
         \item First, we shall prove that
               ...
               induction on $n$. Statement \eqref{1_1_19_1} holds for the base
               ...
               Now we shall show by induction on $n$ that
         ...
         \item For a positive integer $n$, consider the statement
               ...
               We shall use induction on $n$ to prove \eqref{1_1_19_3}. Observe
               that
               \begin{align*}
                  x^{-1} &= (x)^{-1} \\
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%1.2.3%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
   \item[1.2.3]   Use the generators and relations above to show that every
      ...
      \begin{align*}
         x^2 &= (sr^i)(sr^i) \\
             &= s((r^is)r^i) &[\text{Associativity}] \\
             &= s(sr^{-i})r^i &[\text{Lemma 2}] \\
             &= (ss)(r^{-i}r^i) &[\text{Associativity}] \\
             &= 1.
      \end{align*}
      ...
      $s$ and $r$ are in the group generated by $s$ and $sr$. This follows
      immediately since
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%1.2.4%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
   \item[1.2.4]   If $n = 2k$ is even and $n \ge 4$, show that $z = r^k$ is an
      ...
      of $r$. So we now want to show that $z$ commutes with the latter type of
      elements. Thus consider $sr^i$, with $0 \le i < n$. Then we have that
      \begin{align*}
         z(sr^i) &= (r^k)sr^i = (r^ks)r^i  &[\text{Associativity}] \\
      ...
      already showed that at least one exists, namely $r^k$). As previously
      stated, $y$ must be either of two forms, which we will now investigate:
      ...
      that $y$ commutes with $s$; thus,
      \begin{align*}
         sr^{-i} &= r^is &[\text{Lemma 2}] \\
          &= ys = sy = sr^i.
      \end{align*}
      Use left cancellation on the resulting equality $sr^{-i} = sr^i$ to
      obtain $r^{-i} = r^i$. Multiply the preceding equality by $r^i$ to obtain
      $r^{2i} = 1$. It follows by Lemma 1 that $n = 2k \mid 2i$, or
      equivalently, $k \mid i$, so that $i$ is a multiple of $k$. Since $i$ is
      positive and less than $n$, and since $k = n/2$, it follows that the only
      multiple of $k$ in the interval $[1, n)$ is $k$. Thus $k = i$, and we have
      that $y = r^i = r^k = z$.
   ...
   \item[1.3.15]  Prove that the order of an element in $S_n$ equals the least
      ...
      \begin{align*}
         \sigma^r &= (\sigma_1\cdots\sigma_m)^r \\
               &= {\sigma_1}^r\cdots{\sigma_m}^r &[\text{Lemma 3}] \\
